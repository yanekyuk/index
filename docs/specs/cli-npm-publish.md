...
A build script (`packages/cli/scripts/build.ts`) performs cross-compilation:
...
A publish script (`packages/cli/scripts/publish.ts`) automates the release:
...
8. The main `packages/cli/package.json` has `private: true` removed and includes `optionalDependencies` for all platform packages.
