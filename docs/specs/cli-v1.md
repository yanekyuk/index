---
title: "CLI v1 — login and chat"
type: spec
tags: [cli, auth, chat, h2a, sse, streaming]
created: 2026-03-30
updated: 2026-03-30
---
...
**One-shot mode** (message provided as argument):
...
**Interactive REPL mode** (no message argument, no `--list` flag):
...
Resumes a specific chat session. Works in both one-shot and REPL modes. The session ID is passed as `sessionId` in the stream request body.

### `index chat --list`

1. Calls `GET /api/chat/sessions` with auth header.
2. Prints a table of sessions: ID, title, created date.
3. Exits 0.
...
- The binary name is `index` (via `bin` field in package.json or bun build --compile). Distributed via `npm install -g @indexnetwork/cli`.
...
3. `index chat "hello"` sends a message and prints the streamed response.
4. `index chat` (no args) enters REPL mode and supports multi-turn conversation.
5. `index chat --session <id>` resumes an existing session.
6. `index chat --list` prints a formatted table of sessions.
7. All commands exit with code 1 and a helpful message when not authenticated.
8. 401 responses trigger a "Session expired" message.
9. Network errors (server unreachable) produce a clear error message.
10. Unit tests cover: argument parsing, credential read/write, SSE event parsing, error handling.
