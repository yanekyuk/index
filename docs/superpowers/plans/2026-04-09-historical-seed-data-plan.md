# Historical Figure Seed Data Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Replace the 78 generic seed personas with 50 deceased historical figures, each with a personal agent and API key for testing.

**Architecture:** Full rewrite of `test-data.ts` with historically accurate personas. New personal agent + API key creation phase in `db-seed.ts` using existing `AgentTokenAdapter`. Plaintext keys written to `.seed-api-keys.json`.

**Tech Stack:** TypeScript, Drizzle ORM, Bun
...
|------|--------|---------------|
| `backend/src/cli/test-data.ts` | Rewrite | 50 historical figure personas (data only) |
| `backend/src/cli/db-seed.ts` | Modify | Personal agent + API key creation loop, key file output |
| `backend/.gitignore` | Modify | Add `.seed-api-keys.json` |

---

### Task 1: Replace test data with 50 historical figures

**Files:**
- Modify: `backend/src/cli/test-data.ts`

This is a full rewrite of the data array. The interfaces (`SeedProfile`, `TesterPersona`) stay identical. Only the data and `TESTER_PERSONAS_MAX` change.

- [ ] **Step 1: Replace the entire `test-data.ts` file**

Replace the contents of `backend/src/cli/test-data.ts` with the following. The interfaces at the top are unchanged; the array is completely new.
...
export const TESTER_PERSONAS: TesterPersona[] = [
  ...
  // ── Makers & Craftspeople ──────────────────────────────────────────────────
  {
    name: 'Hephaestus-inspired: Benvenuto Cellini',
...
Run: `cd backend && npx tsc --noEmit src/cli/test-data.ts`
Expected: No errors

- [ ] **Step 3: Commit**

```bash
git add backend/src/cli/test-data.ts
git -c commit.gpgsign=false commit -m "feat(seed): replace generic personas with 50 historical figures"
```

---

### Task 2: Add `.seed-api-keys.json` to gitignore
...
Add this line to the end of `backend/.gitignore`:
...
Run: `cd backend && git check-ignore .seed-api-keys.json`
Expected: `.seed-api-keys.json`

- [ ] **Step 3: Commit**

```bash
git add backend/.gitignore
git -c commit.gpgsign=false commit -m "chore: gitignore .seed-api-keys.json"
```

---

### Task 3: Add personal agent + API key creation to seed script
...
At the top of `backend/src/cli/db-seed.ts`, add these imports alongside the existing ones:

```typescript
import { agentTokenAdapter } from '../adapters/agent-token.adapter';
import { agentTransports } from '../schemas/database.schema';
```

Note: `agents` and `agentPermissions` are already imported via the existing `import { ... } from '../schemas/database.schema'` line. `agentTransports` is not currently imported — add it to the existing destructure.

Also add `fs` for writing the key file:

```typescript
import { writeFile } from 'node:fs/promises';
```

- [ ] **Step 2: Define the personal agent permission set**

Below the existing `SYSTEM_AGENT_DEFS` constant (around line 51), add:
...
- [ ] **Step 3: Define the key record type and collection array**

Below the new constant, add:
...
- [ ] **Step 4: Add the personal agent creation phase**

In the `seedDatabase()` function, after the system agent seeding block (after line ~440 where `console.log` says system agents ready), add the personal agent + API key creation phase:
...
Run: `cd backend && npx tsc --noEmit src/cli/db-seed.ts`
Expected: No errors
...
git -c commit.gpgsign=false commit -m "feat(seed): create personal agents and API keys for historical figures"
```

---

### Task 4: End-to-end seed verification

- [ ] **Step 1: Run the seed script against a local database**

Run: `cd backend && bun run db:seed -- --confirm --personas=5`

Expected output includes:
- 5 historical figures created (Miyamoto Musashi, Tomoe Gozen, Sun Tzu, Joan of Arc, Ada Lovelace)
- Profile embedding for each
- Intent creation for each
- 5 personal agents created with API keys
- `.seed-api-keys.json` written with 5 entries

- [ ] **Step 2: Verify the key file contents**

Run: `cat backend/.seed-api-keys.json | head -20`

Expected: JSON array with 5 objects, each having `name`, `email`, `userId`, `agentId`, and `apiKey` (64-char string).

- [ ] **Step 3: Verify personal agents exist in database**

Run: `cd backend && bun -e "
import db from './src/lib/drizzle/drizzle';
import { agents } from './src/schemas/database.schema';
import { eq } from 'drizzle-orm';
const personalAgents = await db.select().from(agents).where(eq(agents.type, 'personal'));
console.log('Personal agents:', personalAgents.length);
for (const a of personalAgents) console.log(' ', a.name, a.status);
process.exit(0);
"`

Expected: 5 personal agents, all active.

- [ ] **Step 4: Verify an API key authenticates**

Use one of the keys from `.seed-api-keys.json`:

```bash
KEY=$(cat backend/.seed-api-keys.json | bun -e "const d = await Bun.stdin.json(); console.log(d[0].apiKey)")
curl -s -H "Authorization: Bearer $KEY" http://localhost:3001/api/me | head -20
```

Expected: Returns the user profile for the first persona (Miyamoto Musashi), not a 401.
