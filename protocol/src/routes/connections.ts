...
import { users, userConnectionEvents, intents, intentIndexes, indexMembers } from '../schemas/database.schema';
...
export const getConnectionsByUser = async (req: AuthRequest, res: Response) => {
  try {
    ...
    const filteredConnections = Array.from(connectionStates.values()).filter(conn => {
      switch (type) {
        case 'inbox':
          // Show REQUEST if user is RECEIVER
          return conn.currentStatus === 'REQUEST' && !conn.isInitiator;
        case 'pending':
          // Show REQUEST if user is INITIATOR
          return conn.currentStatus === 'REQUEST' && conn.isInitiator;
        case 'history':
