...
export class ChatAgent {
   ...
   * Also detects XML-style hallucinations (e.g. `<intent_proposal ... />`).
   *
  ...
  private detectHallucinatedBlock(
    ...
    // Check for hallucinated intent_proposal (code block format)
    ...
    // Check for hallucinated intent_proposal (XML tag format)
    // LLMs (especially Gemini) sometimes emit <intent_proposal description="..." />
    // or <confirm_intent_proposal .../> instead of using the tool.
    if (!hasSuccessfulCreateIntent) {
      const xmlIntentMatch = text.match(/<intent_proposal[^>]*description="([^"]+)"[^>]*\/?>/);
      if (xmlIntentMatch) {
        return { type: "intent_proposal", tool: "create_intent", description: xmlIntentMatch[1] };
      }
      const xmlConfirmMatch = text.match(/<confirm_intent_proposal[^>]*\/?>/);
      if (xmlConfirmMatch) {
        return { type: "intent_proposal", tool: "create_intent", description: "confirm intent" };
      }
    }

    // Check for hallucinated opportunity blocks (code block format)
    ...
    // Check for hallucinated opportunity blocks (XML tag format)
    if (!hasSuccessfulCreateOpportunities) {
      const xmlOppMatch = text.match(/<opportunity[^>]*name="([^"]+)"[^>]*\/?>/);
      if (xmlOppMatch) {
        return { type: "opportunity", tool: "create_opportunities", description: xmlOppMatch[1] };
      }
    }
   ...
   * Also strips XML-style hallucinated tags (e.g. `<intent_proposal ... />`).
  ...
  private stripUnbackedBlocks(
    ...
    // Strip XML tag format — LLMs (especially Gemini) sometimes hallucinate
    // <intent_proposal .../>, <confirm_intent_proposal .../>, <opportunity .../> tags
    if (!hasSuccessfulCreateIntent) {
      const next = result.replace(/<\/?(?:intent_proposal|confirm_intent_proposal)[^>]*\/?>/g, "");
      removedBlock ||= next !== result;
      result = next;
    }
    if (!hasSuccessfulCreateOpportunities) {
      const next = result.replace(/<\/?opportunity[^>]*\/?>/g, "");
      removedBlock ||= next !== result;
      result = next;
    }
  ...
  async streamRun(
    ...
    while (iterationCount < HARD_ITERATION_LIMIT) {
      ...
      // ── No tool calls → check for hallucinated blocks ─────────────────
      // LLMs sometimes write ```intent_proposal / ```opportunity blocks
      // or XML-style <intent_proposal .../> tags directly instead of
      // calling the corresponding tool. These lack valid data and won't
      // work in the frontend. Detect this and force a correction iteration.
