...
mock.module("../src/adapters/cache.adapter", () => ({
...
mock.module("../src/adapters/integration.adapter", () => ({
  ComposioIntegrationAdapter: class {
    getIntegrationStatus = mock(async () => ({}));
  },
}));

mock.module("../src/adapters/database.adapter", () => ({
...
mock.module("../src/adapters/embedder.adapter", () => ({
  EmbedderAdapter: class {
    embed = mock(async () => []);
  },
}));

mock.module("../src/adapters/scraper.adapter", () => ({
  ScraperAdapter: class {
    scrape = mock(async () => ({ title: "", content: "" }));
  },
}));

mock.module("../src/queues/intent.queue", () => ({
  intentQueue: {
    add: mock(async () => {}),
    process: mock(() => {}),
  },
...
mock.module("../src/services/contact.service", () => ({
  contactService: {
...
mock.module("../src/services/integration.service", () => ({
  IntegrationService: class {
    importContacts = mock(async () => ({ imported: 0, skipped: 0, newContacts: 0, existingContacts: 0 }));
  },
}));

mock.module("../src/lib/parallel/parallel", () => ({
  enrichUserProfile: mock(async () => ({})),
}));

import { createDefaultProtocolDeps } from "../src/protocol-init.js";
