...
- **Candidate sections** (only if `list_opportunities` returned candidates):
  ...
  - For each **introducer** candidate: render the line as a community intent — `[{Name}]({profileUrl}) — {their need, 1–2 sentences from mainText}. {short closing phrase}, make intro`. **DO link the name** to `profileUrl` (same Telegram-or-index.network resolution as direct). **Do NOT link the opportunity** — no `acceptUrl`. The trailing `make intro` is plain text, not a hyperlink. The connect/accept link belongs only to direct candidates; for introducer candidates the user replies to the agent if they want to act.
