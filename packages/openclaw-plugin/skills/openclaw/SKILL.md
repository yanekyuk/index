...
If `register_agent` fails with an error whose message contains the substring `can only manage its own registration`, the caller is already agent-bound. Retry once with `add_webhook_transport`, passing the same `url`, `secret`, and `events`. If `add_webhook_transport` then fails with `requires an authenticated agent`, STOP and tell the user exactly this:
