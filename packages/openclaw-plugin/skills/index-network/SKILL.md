...
> Background polling is on. I'll check for new negotiation turns and opportunities every 30 seconds and handle them silently. You can disable negotiations any time by setting `plugins.entries.indexnetwork-openclaw-plugin.config.negotiationMode` to `disabled`.
