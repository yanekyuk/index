...
describe('Opportunity Graph', () => {
  ...
  // ─── Introducer gating lifecycle ─────────────────────────────────────────────
  describe('introducer gating lifecycle', () => {
    test('latent opportunity: gate blocks → approve → queue fires → negotiation runs → non-introducers notified', async () => {
      // Stateful mock DB: tracks actors so an approve mutation is visible to the
      // subsequent negotiate_existing call (chained via the queueNegotiateExisting callback).
      ...
      // Returns acceptance so the non-introducer notification branch fires.
      const mockNegotiationGraph = {
        invoke: async (input: unknown) => {
          negotiationInvocations.push(input);
          return {
      ...
      const mockHyde = { invoke: async () => ({ hydeEmbeddings: {} }) };
      const queueNotification = async (_oppId: string, userId: string) => {
        notifiedUserIds.push(userId);
      };
      ...
      // Simulate BullMQ: the callback triggers negotiate_existing on the same compiled graph,
      // exactly like the backend queue worker (handleNegotiateExisting).
      ...
      // ─── Phase 1: gate is armed (approved: false). No approve_introduction yet. ────
      // Note: the gate test at line 1912 already exercises the negotiate-node skip.
      // Here we assert only that no negotiation has run yet before approval.
      expect(negotiationInvocations).toHaveLength(0);

      // ─── Phase 2: introducer approves → callback should fire ──────────────────────
      await compiledGraph.invoke({
        userId: 'introducer-user' as Id<'users'>,
