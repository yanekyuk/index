...
export function createUtilityTools(defineTool: DefineTool, deps: ToolDeps) {
  ...
  const readDocs = defineTool({
    name: "read_docs",
    description:
      "Returns the protocol's business logic documentation: entity model, intent lifecycle, opportunity lifecycle, discovery mechanics, and key workflows. Call this when you need to understand how the system works or explain it to the user.",
    querySchema: z.object({
